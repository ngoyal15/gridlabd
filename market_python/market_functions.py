...
class Market :
    ...
    def sell(self,quantity,price=[],response=0.0) :
        ...
        if price is None:
            self.S.append([price,round(quantity,self.Qprec),response])
        elif quantity is None:
            self.S.append([round(price,self.Pprec),quantity,response])
        else:
            self.S.append([round(price,self.Pprec),round(quantity,self.Qprec),response])
        self.m |= ( response != 0 )
        self.status = 1
    ...
    def buy(self,quantity,price=[],response=0.0,active=False) :

        if price == [] :
            price = self.Pmax
        assert (response<=0), 'buy response must be non-positive'
        if price is None:
            self.D.append([price,round(quantity,self.Qprec),response])
        elif quantity is None:
            self.D.append([round(price,self.Pprec), quantity, response])
        else:
            self.D.append([round(price,self.Pprec),round(quantity,self.Qprec),response])
        self.m |= ( response != 0 )
        self.status = 1
