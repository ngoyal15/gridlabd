...
EXPORT_METHOD(building,connect);
...
building::building(MODULE *module)
{
	if (oclass==NULL)
	{
		...
		oclass = gld_class::create(module,"building",sizeof(building),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_AUTOLOCK|PC_OBSERVER);
		if (oclass==NULL)
...
int building::connect(char *buffer, size_t len)
{
	printf("building::connect(char *buffer='%s', size_t len=%d)\n", buffer, len);
	if ( len==0 )
	{
		message msg(buffer);
		...
		{
			error("building::connect(char *buffer='%s', size_t len=%d): action is not valid (action='%s')", buffer, len, action);
			return 0;
		}
		if ( source==NULL )
		{
			error("building::connect(char *buffer='%s', size_t len=%d): source is not found", buffer, len);
			return 0;
		}
		gld_object *agent = find_object((char*)source);
		if ( agent==NULL || !agent->is_valid() )
		{
			error("building::connect(char *buffer='%s', size_t len=%d): agent is not valid", buffer, len);
			return 0;
		}
		sell = gld_property(agent,"sell");
		if ( !sell.is_valid() )
		{
			error("building::connect(char *buffer='%s', size_t len=%d): agent does not implement 'sell' method", buffer, len);
			return 0;
		}
		return 1;
	}
...
TIMESTAMP building::presync(TIMESTAMP t1)
{
	if ( sell.is_valid() )
	{
		gld_clock now;
...
TIMESTAMP building::postsync(TIMESTAMP t1)
{
	return TS_NEVER;
}

TIMESTAMP building::commit(TIMESTAMP t1, TIMESTAMP t2)
{
	// TODO
	return TS_NEVER;
}

ENDUSELOAD *building::add_load(OBJECT *obj, bool is220)
{
